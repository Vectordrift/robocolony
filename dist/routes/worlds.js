...
export async function worldRoutes(app) {
    // List worlds
    app.get('/api/worlds', async (_request, _reply) => {
    ...
    app.post('/api/worlds/:id/join', async (request, reply) => {
        ...
        for (const pos of worldMap.startingPositions) {
            const tooClose = occupied.some((s) => hexDistance(s, pos) < MIN_SPAWN_SPACING);
            if (!tooClose) {
                startingHex = pos;
                break;
            }
        ...
        await db
            .update(hexes)
            .set({ settlementId })
            .where(and(eq(hexes.worldId, worldId), eq(hexes.x, startingHex.q), eq(hexes.y, startingHex.r)));
    ...
    `);
        ...
        const unclaimedLand = await db
            .select({ count: sql `count(*)` })
            .from(hexes)
            .where(and(eq(hexes.worldId, worldId), sql `terrain NOT IN ('ocean', 'coast')`, sql `settlement_id IS NULL`));
    ...
    app.delete('/api/worlds/:id/colony', {
        ...
        await db
            ...
            .where(and(eq(hexes.worldId, worldId), sql `settlement_id IN (SELECT id FROM settlements WHERE colony_id = ${colonyId})`));
        // 3. Settlements
