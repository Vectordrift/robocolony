...
export async function messageRoutes(app) {
    ...
    app.get('/api/worlds/:id/messages', {
        ...
        const conditions = [
            ...
            or(eq(messages.toColony, colony.id), eq(messages.fromColony, colony.id)),
        ];
    ...
    app.post('/api/worlds/:id/messages/:msgId/read', {
        preHandler: requireAuth,
    }, async (request, reply) => {
        const colony = request.colony;
        const worldId = colony.worldId;
        ...
        const rows = await db
            ...
            .from(messages)
            .where(and(eq(messages.id, msgId), eq(messages.worldId, worldId)))
            .limit(1);
        if (rows.length === 0) {
