...
export const SCORE_UNIT = { scout: 2, militia: 5, soldier: 10, siege: 15, settler: 3 };
...
export function resolveTick(colonies, settlements, units, hexes, actions = [], combatSeed, worldId, currentTick, agreements) {
    ...
    const combatHexesThisTick = new Set();
    ...
    // Units from different colonies sharing a hex fight automatically.
    {
        for (const unit of updatedUnits) {
            ...
            const coloniesOnHex = updatedUnits.filter(other => other.health > 0 && other.hexX === unit.hexX && other.hexY === unit.hexY);
    ...
    for (const colony of updatedColonies) {
        ...
        for (const unit of myUnits) {
            ...
            if (hasEnemiesOnHex)
                continue;
            if (combatHexesThisTick.has(unitHexKey))
                continue;
            // Calculate barracks bonus
            let barracksBonus = 0;
            for (const b of garrisonSettlement.buildings) {
                if (b.type === 'barracks')
                    barracksBonus += BARRACKS_HEALING_BONUS * b.level;
            }
