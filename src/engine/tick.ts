...
export function resolveCombat(
  ...
  if (settlements) {
    for (const settlement of settlements) {
      ...
      if (!combatHex || combatHex.length < 2) continue;

      // Check if the settlement's colony still has surviving units on this hex
      const defenderAlive = survivingUnits.some(
        u => u.colonyId === settlement.colonyId && u.hexX === settlement.hexX && u.hexY === settlement.hexY
      );
      if (defenderAlive) continue;

      // Check if any enemy units survived on this hex
      const attackerUnits = survivingUnits.filter(
        u => u.colonyId !== settlement.colonyId && u.hexX === settlement.hexX && u.hexY === settlement.hexY
      );
