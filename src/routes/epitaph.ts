...
interface EpitaphParams {
  Params: { id: string };
}

type EpitaphEventRow = {
  id: string;
  tick: number;
  type: string;
  public: boolean;
...
function classifyDeathCause(
  ...
  recentEvents: EpitaphEventRow[],
): 'conquest' | 'famine' | 'desertion' | 'neglect' | 'unknown' {
  const lowerReason = (deathReason ?? '').toLowerCase();

  if (lowerReason.includes('captured')) return 'conquest';
  if (recentEvents.some(event => event.type === 'famine')) return 'famine';
  if (recentEvents.some(event => event.type === 'desertion')) return 'desertion';
...
export async function epitaphRoutes(app: FastifyInstance) {
  app.get<EpitaphParams>('/api/worlds/:id/epitaph', {
    ...
    const colonyData = colonyRows[0] as typeof colonyRows[0] & {
    ...
    const recentEventsRaw = await db
      ...
      .where(and(
        ...
        gte(events.tick, recentTickStart),
        lte(events.tick, deathTick),
        or(
          eq(events.public, true),
          sql`${colony.id} = ANY(${events.visibility})`,
        ),
      ))
      .orderBy(desc(events.tick));

    const recentEvents: EpitaphEventRow[] = recentEventsRaw.map((event) => ({
      id: event.id,
      tick: event.tick,
      type: event.type,
      public: event.public,
      ...
      publicData: (event.publicData ?? null) as Record<string, unknown> | null,
    }));

    const eliminationEvent = recentEvents.find(event =>
      event.type === 'colony_eliminated' || event.type === 'colony_dead',
    );
    const attackerColonyId = typeof eliminationEvent?.data.eliminatedBy === 'string'
      ? eliminationEvent.data.eliminatedBy
      : undefined;

    let attackerColony: { id: string; name: string } | null = null;
    if (attackerColonyId) {
      const attackerRows = await db
        ...
        .where(and(
          ...
          eq(colonies.id, attackerColonyId),
        ))
        .limit(1);
